...
def neg_log_likelihood(rates, spikes, zero_warning=True):
    ...
    ), f"""neg_log_likelihood: Rates and spikes should
    be of the same shape. spikes: {spikes.shape}, rates: {rates.shape}"""
    ...
    if np.any(rates == 0):
        if zero_warning:
            logger.warning(
                """neg_log_likelihood: Zero rate predictions found.
                Replacing zeros with 1e-9"""
            )
        rates[rates == 0] = 1e-9
...
def bits_per_spike(rates, spikes):
    ...
    of the rate predictions and the null model
    (i.e. predicting mean firing rate of each neuron)
...
def velocity_decoding(
    ...
    """
    flatten3d = (
        lambda x: x.reshape(-1, x.shape[2]) if (len(x.shape) > 2) else x
    )  # noqa: E731
    decoding_r2s = []
