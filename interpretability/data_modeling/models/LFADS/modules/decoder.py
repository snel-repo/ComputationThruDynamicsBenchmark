...
from .recurrent import ClippedGRUCell, ClippedNODECell
...
class Decoder(nn.Module):
    def __init__(self, hparams):
        ...
        if hps.gen_type == "RNN":
            ...
            self.rnn = DecoderNODE(hparams=hparams)
...
class DecoderNODE(nn.Module):
    def __init__(self, hparams):
        super().__init__()
        self.hps = hparams
        self.cell = DecoderNODECell(hparams=hparams)
...
class DecoderNODECell(nn.Module):
    def __init__(self, hparams):
        super().__init__()
        self.hparams = hps = hparams
        # Create the generator
    ...
    def forward(self, input, h_0, sample_posteriors=True):
        ...
        if self.use_con:
            ...
            gen_input = ext_input_step
        # compute and store the next
        gen_state_drop = gen_state + 0.1 * self.gen_cell(gen_input, gen_state)
        factor = self.fac_linear(gen_state_drop)
        hidden = torch.cat(
            [gen_state_drop, con_state, co_mean, co_std, gen_input, factor], dim=1
        )
        return hidden
